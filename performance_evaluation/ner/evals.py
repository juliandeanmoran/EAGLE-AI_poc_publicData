...
class F1Evaluator:
    ...
    def print_report(report: Dict[str, Any]) -> None:
        ...
        # Variants
        # vm = report.get('variants', {})
        # print("\nVariants:")
        # for key, title in [('genomic', 'Genomic (g.)'), ('coding', 'cDNA (c.)'), ('protein', 'Protein (p.)'), ('all', 'All combined')]:
        #     if key in vm:
        #         m = vm[key]
        #         print(f"  - {title:<15} P: {m.get('precision', 0):.3f}, R: {m.get('recall', 0):.3f}, F1: {m.get('f1', 0):.3f}  (TP={m.get('tp', 0)}, FP={m.get('fp', 0)}, FN={m.get('fn', 0)})")
